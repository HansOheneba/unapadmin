...
export default function ReturnsPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      <Tabs value={status} onValueChange={(v) => setStatus(v as ReturnStatus | "")}>
        <TabsList>
          {STATUS_TABS.map((tab) => (
        ...
        <Table>
          ...
          <TableBody>
            {isLoading
              ...
              : data?.data.map((r) => (
                  <TableRow key={r.id}>
                    <TableCell className="font-mono text-xs text-zinc-600">{r.id}</TableCell>
                    <TableCell className="font-medium text-zinc-900">{r.orderNumber}</TableCell>
                    <TableCell className="text-zinc-600">{r.customer.name ?? r.customer.email}</TableCell>
                    <TableCell className="text-zinc-600 capitalize">{r.reason.replace(/_/g, " ").toLowerCase()}</TableCell>
                    <TableCell className="text-zinc-600">{r.items.length}</TableCell>
