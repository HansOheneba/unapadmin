...
export default function CustomerDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const avgOrderValue = data.totalOrders > 0 ? data.totalSpent / data.totalOrders : 0;

  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-semibold text-zinc-900">Customer</h1>
        <Link href="/dashboard/customers"><Button variant="outline">Back</Button></Link>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-zinc-100 rounded-lg p-6 space-y-3">
          ...
          <div className="space-y-2 text-sm">
            <div><p className="text-zinc-500">Name</p><p className="text-zinc-900 font-medium">{data.name ?? "N/A"}</p></div>
            <div><p className="text-zinc-500">Email</p><p className="text-zinc-800">{data.email}</p></div>
            <div><p className="text-zinc-500">Phone</p><p className="text-zinc-800">{data.phone ?? "N/A"}</p></div>
            <div><p className="text-zinc-500">Country</p><p className="text-zinc-800">{data.country ?? "N/A"}</p></div>
            <div><p className="text-zinc-500">Joined</p><p className="text-zinc-800">{formatDate(data.createdAt)}</p></div>
          </div>
        </div>
        ...
        <div className="lg:col-span-2 grid grid-cols-3 gap-4">
          {[
            ...
            <div key={stat.label} className="bg-white border border-zinc-100 rounded-lg p-5">
        ...
        <Table>
          ...
          <TableBody>
            {data.orders.length === 0 ? (
              <TableRow>
                <TableCell colSpan={5} className="text-center text-zinc-400 py-8">No orders yet.</TableCell>
              </TableRow>
            ) : (
              data.orders.map((order) => (
                <TableRow key={order.id}>
                  <TableCell className="font-medium">{order.orderNumber}</TableCell>
                  <TableCell className="text-zinc-600">{formatDate(order.createdAt)}</TableCell>
